...
def modify():
    #pahse 1 - detect skin, only estuary and confluence are supported
    skin = xbmc.getSkinDir() #simplified
    if skin not in SKINS:
        xbmcgui.Dialog().ok('Unsupported', 'Sorry, your skin is not supported.')
        return
    ...
    if xbmcvfs.exists(destination + "/"):
        xbmcgui.Dialog().ok(newName, 'Copy already exist, uninstall it first.')
        xbmc.executebuiltin("ActivateWindow(10040,addons://user/xbmc.gui.skin,return)")
        return
    if not xbmcvfs.exists(destsettings + "/"):
        copy_settings = xbmcgui.Dialog().yesno(SKINS[skin], 'Copy settings as well?')
    else:
        copy_settings = False
    ...
    if skin == 'skin.estuary':
        ...
        if len(content) != 1:
            ...
            xbmcgui.Dialog().ok(SKINS[skin], 'TODO')
            return
        content = content[0]
        ...
        if len(content) != 1:
            ...
            print(len(content))
            cleanup(copy_settings, destination, destsettings)
            xbmcgui.Dialog().ok(SKINS[skin], 'TODO1')
            return
        content = content[0]
        ...
        if len(toRemove) == 0:
            cleanup(copy_settings, destination, destsettings)
            xbmcgui.Dialog().ok(SKINS[skin], 'TODO2')
            return
    ...
    try:
        ...
        #xbmc.executebuiltin("ActivateWindow(10040,addons://user/xbmc.gui.skin,return)")
        if xbmcgui.Dialog().yesno(SKINS[skin], 'Done. Try to set?'):
            xbmc.executeJSONRPC('{"jsonrpc":"2.0","method":"Settings.SetSettingValue","id":1,"params":{"setting":"lookandfeel.skin","value":"'+newSkin+'"}}')
            xbmc.executebuiltin('SendClick(11)')
            time.sleep(1)
            xbmc.executebuiltin( 'XBMC.ReloadSkin()' )
            xbmcgui.Dialog().ok(newName, 'Done')
    except:
        xbmcgui.Dialog().ok("skylink","Kodi web interface wasn't enabled. Restart Kodi and Enable Skin in My Addons.")
        xbmc.executebuiltin("ActivateWindow(10040,addons://user/xbmc.gui.skin,return)")
